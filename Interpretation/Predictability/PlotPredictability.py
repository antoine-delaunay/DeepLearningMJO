...
def Plot2Phases(X, XM, significance, feature_idx, phase1, phase2, levels_means, levels_anoms, n_days):
    ...
    ax20.set_title(r"\textbf{c. }"+'Weak - Certain (' + str(n_days[0,phase1]) + ' days) vs. Uncertain (' + str(n_days[1,phase1]) + ' days)', loc = 'left')
    ...
    ax21.set_title(r"\textbf{d. }"+'Weak - Certain (' + str(n_days[0,phase2]) + ' days) vs. Uncertain (' + str(n_days[1,phase2]) + ' days)', loc = 'left')    
    ...
    ax30.set_title(r"\textbf{e. }"+'Strong - Certain (' + str(n_days[2,phase1]) + ' days) vs. Uncertain (' + str(n_days[3,phase1]) + ' days)', loc = 'left')    
    ...
    ax31.set_title(r"\textbf{f. }"+'Strong - Certain (' + str(n_days[2,phase2]) + ' days) vs. Uncertain (' + str(n_days[3,phase2]) + ' days)', loc = 'left')    
...
if __name__ == '__main__':
    ...
    X0, y0, y_T_OUTPUT, phase0 = torch.cat((X0_train, X0_test),0), np.concatenate((y0_train, y0_test), axis=0), np.concatenate((y_T_OUTPUT_train, y_T_OUTPUT_test), axis=0), torch.cat((phase0_train, phase0_test), 0).numpy()
    ...
    print('Define certain and uncertain events')
    ...
    #Get index of events according to strength and certainty
    idx_wc = np.where(((cov[:,0]<=q30[0]) & (cov[:,1]<=q30[1])) & (amp0<=1.0))[0]
    idx_wu = np.where(((cov[:,0]>=q70[0]) & (cov[:,1]>=q70[1])) & (amp0<=1.0))[0]
    idx_sc = np.where(((cov[:,0]<=q30[0]) & (cov[:,1]<=q30[1])) & (amp0>=1.0))[0]
    idx_su = np.where(((cov[:,0]>=q70[0]) & (cov[:,1]>=q70[1])) & (amp0>=1.0))[0]
    
    ph_wc, ph_wu, ph_sc, ph_su = phase0[idx_wc], phase0[idx_wu], phase0[idx_sc], phase0[idx_su]
    X0_wc, X0_wu, X0_sc, X0_su = X0[idx_wc], X0[idx_wu], X0[idx_sc], X0[idx_su]
    mu_wc, mu_wu, mu_sc, mu_su = mu[idx_wc], mu[idx_wu], mu[idx_sc], mu[idx_su]

    X = np.zeros((4,8,7,17,144))
    for i in range(8):
        idx = np.where(ph_wc==i+1)[0]
        X[0,i,:,:,:] = X0_wc[idx].mean(axis=0)
        idx = np.where(ph_wu==i+1)[0]
        X[1,i,:,:,:] = X0_wu[idx].mean(axis=0)
        idx = np.where(ph_sc==i+1)[0]
        X[2,i,:,:,:] = X0_sc[idx].mean(axis=0)
    ...
    #Compute anomalies maps between certain and uncertain events
    ...
    ''' For each point of the map and feature, we compute the significance 
        of the difference between weak(strong) certain and weak (strong) uncertain forecasts
        two-tailed student's t-test with 95% of confidence level
    '''
    
    for p in tqdm(range(8)):
        idx = np.where(phase0==p+1)[0]
        idx_wc = np.where(ph_wc==p+1)[0]
        idx_wu = np.where(ph_wu==p+1)[0]
        idx_sc = np.where(ph_sc==p+1)[0] 
        idx_su = np.where(ph_su==p+1)[0] 
        
        n_days[0,p] = idx_wc.shape[0]
        n_days[1,p] = idx_wu.shape[0]
        n_days[2,p] = idx_sc.shape[0]
        n_days[3,p] = idx_su.shape[0]
            
        for k in range(7):
            for i in range(17):
                for j in range(144):
                     t_stats[0,p,k,i,j], p_values[0,p,k,i,j] = stats.ttest_ind(X0_wc[idx_wc,k,i,j], X0_wu[idx_wu,k,i,j], axis=0)      
                     t_stats[1,p,k,i,j], p_values[1,p,k,i,j] = t_stats[0,p,k,i,j], p_values[0,p,k,i,j]
                     t_stats[2,p,k,i,j], p_values[2,p,k,i,j] = stats.ttest_ind(X0_sc[idx_sc,k,i,j], X0_su[idx_su,k,i,j], axis=0)      
    ...
    Plot2Phases(X_, XM, significance, 3, 3, 4, levels_means, levels_anoms, n_days)        
    Plot2Phases(X_, XM, significance, 3, 5, 6, levels_means, levels_anoms, n_days)        
    Plot2Phases(X_, XM, significance, 5, 3, 4, levels_means, levels_anoms, n_days)
    Plot2Phases(X_, XM, significance, 5, 5, 6, levels_means, levels_anoms, n_days)